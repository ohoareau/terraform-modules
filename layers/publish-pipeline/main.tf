...
module "codebuild-project-publish" {
...
module "codepipeline-publish" {
  ...
  stages = [
    ...
    {name = "Publish", type = "Build", provider = "CodeBuild", inputs = ["SourceArtifact"], outputs = [], config = {ProjectName = module.codebuild-project-publish.name}},
  ]
  policy_statements = [
...
module "codepipeline-publish-artifacts-policy-for-codebuild-project-publish" {
  source          = "../../modules/codepipeline-artifacts-policy"
  role_name       = module.codebuild-project-publish.role_name
  pipeline_bucket = module.codepipeline-publish.artifacts_bucket_arn
}
...
module "codepipeline-publish-ecr-trigger" {
  ...
  ecr_tag       = "latest"
  pipeline_arn  = module.codepipeline-publish.arn
  pipeline_name = module.codepipeline-publish.name
}
